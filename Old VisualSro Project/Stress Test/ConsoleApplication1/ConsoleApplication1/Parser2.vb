Module Parser2
    ...
    Sub ServerList(ByVal index_ As Integer, ByVal packet As PacketReader)
        packet.Word()
        Dim len As UShort = packet.Word
        Dim nameserver As String = packet.String(len)
        packet.Byte()
        ...
        Do While True
            ...
            If tag = 1 Then
                ...
                Dim servername = packet.String(packet.Word)
                Dim acus As UShort = packet.Word
                Dim maxus As UShort = packet.Word
                Dim state As Byte = packet.Byte

                SendLogin(index_, serverid)
                Exit Do
            Else
                Exit Do
            End If
    ...
    Sub SendLogin(ByVal Index_ As Integer, ByVal serverid As UShort)
        Dim name As String = "stress" & Index_
        Dim pass As String = "stress"
